...
export class CommentLikeService implements ICommentLikeService {
  ...
  async like(userId: number, commentId: number): Promise<ServiceResult> {
    ...
    if ( membership.id !== 0 && ( membership.status === CommunityMemberStatus.BANNED || membership.status === CommunityMemberStatus.PENDING)) {
        return ServiceResultFactory.fail(CommentMessages.cannotLikeComment, HttpStatus.forbidden);
    }
